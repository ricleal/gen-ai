...
def evaluate_results_step(
    ...
    for i, story in enumerate(stories[:10]):
        console.print(f"[dim]    📝 Preparing story {i + 1} for evaluation...[/dim]")
    ...
    if comments:
        console.print(f"[dim]    💬 Preparing comments ({len(comments[:20])}) for evaluation...[/dim]")
    ...
    console.print(f"[dim]    🧠 Sending evaluation prompt to LLM...[/dim]")
    messages = [
        {
            "role": "system",
    ...
    console.print(f"[dim]    🧠 LLM response received. Cleaning and parsing...[/dim]")

    cleaned_response = clean_json_response(response)

    # Try to parse JSON with error handling
    try:
        console.print(f"[dim]    🧠 Parsing LLM response JSON...[/dim]")
        evaluation_dict = json.loads(cleaned_response)

        # Normalize the insights field - convert dicts to strings if needed
        if "insights" in evaluation_dict and isinstance(evaluation_dict["insights"], list):
            normalized_insights = []
            for insight in evaluation_dict["insights"]:
                if isinstance(insight, dict):
                    # Convert dict insight to string
                    insight_str = " ".join(f"{k}: {v}" for k, v in insight.items())
                    normalized_insights.append(insight_str)
                else:
                    normalized_insights.append(str(insight))
            evaluation_dict["insights"] = normalized_insights

        # Normalize relevance_score - handle if it's a list, take the first value or average
        if "relevance_score" in evaluation_dict:
            score = evaluation_dict["relevance_score"]
            if isinstance(score, list) and len(score) > 0:
                # Take the first value if it's a list
                evaluation_dict["relevance_score"] = int(score[0]) if isinstance(score[0], (int, float)) else 5
            elif isinstance(score, (int, float)):
                evaluation_dict["relevance_score"] = int(score)
            else:
                evaluation_dict["relevance_score"] = 5

        # Normalize key_points - ensure it's a list of strings
        if "key_points" in evaluation_dict:
            key_points = evaluation_dict["key_points"]
            if isinstance(key_points, list):
                ...
                evaluation_dict["key_points"] = normalized_points

        # Ensure required fields exist with defaults
        evaluation_dict.setdefault("relevance_score", 5)
        evaluation_dict.setdefault("summary", f"Analysis of {len(stories)} stories about {query}")
        evaluation_dict.setdefault("key_points", ["Analysis completed"])
        evaluation_dict.setdefault("insights", ["Analysis completed"])

    except (json.JSONDecodeError, Exception) as e:
        # If JSON parsing fails, create a basic response
        console.print(f"Warning: JSON parsing/validation failed: {e}")
        console.print(f"Response was: {cleaned_response[:500]}...")
        evaluation_dict = {
            "insights": ["Analysis in progress - model response formatting issue"],
            "relevance_score": 5,
            "summary": f"Found {len(stories)} stories about {query}",
            "key_points": ["Data collection successful, analysis pending"],
        }
...
@DBOS.step()
def generate_follow_ups_step(topic: str, current_findings: List[Dict[str, Any]], iteration: int) -> Optional[str]:
    ...
    try:
        ...
        console.print(f"Warning: JSON parsing failed in generate_follow_ups: {e}")
        return None
...
def should_continue_step(
    ...
    try:
        ...
        print(f"Warning: JSON parsing failed in should_continue: {e}")
        # Default to stopping if we can't parse the response
        return False
...
def synthesize_findings_step(topic: str, all_findings: List[Dict[str, Any]]) -> ResearchReport:
    ...
    try:
        ...
        # Normalize the report field - ensure it's a string
        if "report" in json_response:
            report_value = json_response["report"]
            if isinstance(report_value, list):
                # If it's a list, join the items
                json_response["report"] = "\n\n".join(str(item) for item in report_value)
            elif isinstance(report_value, dict):
                # If it's a dict, convert to string representation
                json_response["report"] = "\n\n".join(f"{k}: {v}" for k, v in report_value.items())
            elif not isinstance(report_value, str):
                # If it's any other type, convert to string
                json_response["report"] = str(report_value)
        else:
            # If report field is missing, create a basic one
            json_response["report"] = f"Research Report on {topic}\n\nCollected {len(all_findings)} sets of findings."

        return ResearchReport(**json_response)
    except (json.JSONDecodeError, Exception) as e:
        console.print(f"[red]Warning: JSON parsing failed in synthesize_findings: {e}[/red] ")
        # Create a basic report if parsing fails
        basic_report = f"Research Report on {topic}\n\nCollected {len(all_findings)} sets of findings.\n\n"
        basic_report += "Note: Full synthesis unavailable due to processing error."
        return ResearchReport(report=basic_report)
